...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .my-listing-card{ background:var(--sand); border:1.5px solid var(--ink); border-radius:6px; padding:16px; margin-bottom:14px; }
  .my-listing-card h4{ font-size:1rem; margin-bottom:4px; }
  .my-listing-card .status-tag{ font-family:'JetBrains Mono', monospace; font-size:0.68rem; text-transform:uppercase; padding:2px 8px; border-radius:3px; display:inline-block; margin-bottom:8px; }
  .status-active{ background:#e2f0e8; color:var(--green); }
  .status-pending{ background:#fbe8cf; color:var(--clay); }
  .img-thumbs{ display:flex; gap:8px; flex-wrap:wrap; margin:10px 0; }
  .img-thumbs img{ width:70px; height:70px; object-fit:cover; border-radius:5px; border:1.5px solid var(--ink); }
  .img-thumb-wrap{ position:relative; }
  .img-thumb-wrap button{ position:absolute; top:-6px; right:-6px; background:var(--clay); color:#fff; border:none; border-radius:50%; width:20px; height:20px; font-size:0.7rem; cursor:pointer; line-height:1; }
  .upload-row{ display:flex; align-items:center; gap:10px; margin-top:8px; }
  .card-thumb{ width:100%; height:140px; object-fit:cover; border-radius:4px; margin-bottom:10px; border:1.5px solid var(--ink); }
...
<header class="page-head">
  <div class="wrap">
    ...
    <p>BizApp ZW is a growing marketplace connecting businesses and merchants with customers searching for what they need. Browse listings across retail, food &amp; beverage, services, tech, and fashion — or list your own business to be found by customers nearby. Pay for your listing in fiat or Pi Network currency, with flexible one-time, monthly, and annual plans.</p>
    <p>Whether you're looking for a carpenter, a caterer, a barber, a mechanic, or any local service, search by keyword or browse by category below. Businesses that list here get a direct WhatsApp contact link, so customers can reach out instantly — no middleman, no delay.</p>
  </div>
</header>

<section class="wrap">
  <div class="filter-bar" style="flex-direction:column; align-items:stretch; gap:10px;">
    <input type="text" id="search-input" placeholder="Search products or services..." style="width:100%; padding:12px 14px; border-radius:6px; border:1.5px solid var(--ink); font-family:'Work Sans', sans-serif; font-size:0.95rem; background:var(--paper);">
...
<section class="cta-section">
  <div class="wrap">
    ...
    <div class="panel" id="my-listings-panel" style="display:none; margin-top:20px;">
      <h3>My listings</h3>
      <p style="font-size:0.9rem; margin-bottom:14px;">Once a listing is active, add photos here — how many you can add depends on your plan.</p>
...
// Get a free key at https://api.imgbb.com/ and paste it here — this is what
// lets image uploads work. Without a real key, uploads will fail.
...
async function loadListings() {
  ...
  try {
    ...
    const filtered = query
      ? listings.filter((l) => {
          const haystack = `${l.business_name || ""} ${l.description || ""} ${l.category || ""}`.toLowerCase();
    ...
    if (filtered.length === 0) {
      state.textContent = query
        ...
        : "No listings yet — be the first to list your business below.";
      state.style.display = "block";
      return;
    }
    ...
    filtered.forEach((l) => {
      ...
      const thumb = images[0]
        ? `<img class="card-thumb" src="${escapeHtml(images[0])}" alt="${escapeHtml(l.business_name || "")}">`
        : "";
      card.innerHTML = `
        ${thumb}
...
function onLoggedIn() {
  ...
  document.getElementById("my-listings-panel").style.display = "block";
  loadMyListings();
}
...
async function loadPricing() {
  try {
    ...
    select.innerHTML = loadedPlans
      ...
      .join("");
    updatePlanDetail();
    const methodsLower = (data.payment_methods || []).map((m) => (m || "").toLowerCase());
    const ecocash = methodsLower.includes("ecocash");
    const ecocashKey = Object.keys(data.payment_details || {}).find((k) => k.toLowerCase() === "ecocash");
    if (ecocash) {
      const ecocashDetail = ecocashKey ? (data.payment_details[ecocashKey] || "") : "";
...
async function loadMyListings() {
  ...
  try {
    ...
    const listings = data.listings || [];
    if (!listings.length) {
      el.innerHTML = `<p style="font-size:0.9rem; color:#666;">You haven't created a listing yet — use the form above.</p>`;
      return;
    }
    el.innerHTML = "";
    listings.forEach((l) => el.appendChild(renderMyListingCard(l)));
  } catch {
    el.innerHTML = `<p style="font-size:0.9rem; color:var(--clay);">Couldn't load your listings — try again shortly.</p>`;
  }
}
...
function renderMyListingCard(l) {
  ...
  const images = Array.isArray(l.images) ? l.images : [];
  const allowed = Number(l.images_allowed || 0);
  ...
  const thumbsHtml = images
    .map(
      ...
      <div class="img-thumb-wrap">
        ...
        <button type="button" onclick="removeListingImage('${l.id}', '${escapeHtml(img)}')">×</button>
      </div>`
    )
    .join("");

  card.innerHTML = `
    <span class="status-tag ${isActive ? "status-active" : "status-pending"}">${l.status}</span>
    <h4>${escapeHtml(l.business_name || "Untitled")}</h4>
    <div class="img-thumbs">${thumbsHtml}</div>
    ${
      isActive
        ? images.length < allowed
          ? `<div class="upload-row">
               <input type="file" accept="image/*" id="file-${l.id}">
               <button class="btn btn-outline" type="button" onclick="uploadListingImage('${l.id}')">Add photo (${images.length}/${allowed})</button>
             </div>
             <div class="result-msg" id="upload-msg-${l.id}" style="font-size:0.85rem; margin-top:6px;"></div>`
          : `<p style="font-size:0.85rem; color:#666;">You've used all ${allowed} photo slot(s) on this plan.</p>`
        : `<p style="font-size:0.85rem; color:#666;">Photos can be added once this listing is approved and active.</p>`
    }
  `;
  return card;
}
...
async function uploadListingImage(listingId) {
  const fileInput = document.getElementById(`file-${listingId}`);
  const msgEl = document.getElementById(`upload-msg-${listingId}`);
  const file = fileInput.files[0];
  if (!file) {
    msgEl.textContent = "Choose a photo first.";
    return;
  }
  if (IMGBB_API_KEY === "PASTE_YOUR_IMGBB_KEY_HERE") {
    msgEl.textContent = "Image hosting isn't set up yet — add your ImgBB API key in the site code.";
    return;
  }
  ...
  try {
    ...
    if (!uploadData.success) {
      msgEl.textContent = "Upload failed — try a different photo.";
      return;
    }
    ...
    const data = await res.json();
    if (!res.ok) {
      msgEl.textContent = data.error || "Couldn't save the image.";
      return;
    }
    loadMyListings();
  } catch {
    msgEl.textContent = "Couldn't reach the server — try again shortly.";
  }
}

async function removeListingImage(listingId, url) {
  try {
    ...
    alert("Couldn't remove the image — try again shortly.");
  }
}
</script>
